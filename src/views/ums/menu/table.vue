<template>
  <div class="app-container">
    ...
    <div class="filter-container">
      <el-button class="filter-item" style="margin-left: 10px;" size="mini" type="primary" icon="el-icon-search" @click="fetchData">刷新</el-button>
    ...
    >
      ...
      <el-table-column label="菜单名称" min-width="10%">
        <template slot-scope="scope">
          {{ scope.row.title }}
        </template>
      </el-table-column>
      ...
      <el-table-column label="Name" min-width="10%">
        <template slot-scope="scope">
          {{ scope.row.name }}
        </template>
      </el-table-column>
      <el-table-column label="图标" min-width="10%">
        <template slot-scope="scope">
          {{ scope.row.icon }}
        </template>
      </el-table-column>
      ...
      <el-table-column label="是否显示" min-width="5%" align="center">
...
import { getPage, del, update } from '@/api/ums/menu'
...
export default {
  ...
  methods: {
    ...
    changeEnable(row) {
      const param = { 'id': row.id, 'hidden': row.hidden }
